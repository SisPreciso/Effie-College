...
const sectionAll = document.querySelector('#data');

const newSectionAll = JSON.parse(sectionAll.value);

console.log(newSectionAll);

newSectionAll.forEach((s) => {
    configEditor(s);
});
...
function sendAnswer(evt, idx) {
    ...
    $.post($('#frm-ans' + idx).attr('action'), {
        ...
        .fail(function (msg) {
            ...
            $('#ans' + idx + '-fail-msg').html(JSON.stringify(msg));
