    ...
    <div class="ficha">
        ...
        <div style="display: flex; flex-wrap: wrap; text-align: center;">
            <div style="flex: 33.3%;">
                ...
                <a href="{{ route('downloadBrief', ['edition' => $edition,'brand' => $user->caso->brand->name]) }}"
                   id="btn-brief"><i class="fa fa-download fa-2x"></i></a>
            </div>
            <div style="flex: 33.3%;">
                <p class="dato">{{ __('Descargar la grabación') }}</p>
                <a href="{{ route('downloadAudio', ['edition' => $edition,'brand' => $user->caso->brand->name]) }}"
                   id="btn-recording"><i class="fa fa-download fa-2x"></i></a>
            </div>
            <div style="flex: 33.3%;">
                <p class="dato">{{ __('Descargar los materiales gráficos') }}</p>
                <a href="{{ route('downloadVisual', ['edition' => $edition,'brand' => $user->caso->brand->name]) }}"
                   id="btn-graphics"><i class="fa fa-download fa-2x"></i></a>
            </div>
        </div>
        <div class="space"></div>
